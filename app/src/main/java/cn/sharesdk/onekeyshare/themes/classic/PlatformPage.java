...
public abstract class PlatformPage extends OnekeySharePage {
	...
	//activity-->(llPage)LinearLayout-->(llPanel)LinearLayout--> top TextView + (title textview, share Platform ，cancel Button)
	//设置背景 title textview的布局布局，cancel button的布局布局，PlatformPagerAdapter类里llPanel(LinearLayout)的背景
	...
	public void onCreate() {
		...
		//LinearLayout llPanel(注：设置分享平台的内容界面背景到PlatformPagerAdapter类里设置)
		...
		//设置textview的布局布局，来实现模拟分享view布局
//		buttonLayout.setBackgroundColor(getContext().getResources().getColor(com.zt.txnews.R.color.bj));
		buttonLayout.setBackgroundColor(Color.GRAY);
		LinearLayout.LayoutParams lp2 = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,110);
		...
		int text_h=buttonLayout.getLayoutParams().height;
		//platform
		...
		int share_content_h=mvp.getLayoutParams().height;

		// cancel button
		lp = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,150);
		lp.setMargins(0,0,0,0);
		LinearLayout buttonLayout2 = new LinearLayout(activity);
		//设置取消按钮的布局布局，来模拟分享view布局
//		buttonLayout2.setBackgroundColor(getContext().getResources().getColor(com.zt.txnews.R.color.bj));
		buttonLayout2.setBackgroundColor(Color.GRAY);
		LinearLayout.LayoutParams lp22 = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,110);
		...
		int cancel_but_h=buttonLayout2.getLayoutParams().height;

		//为llPanel设置lp来居中显示
		int share_view_hight=text_h+share_content_h+cancel_but_h;
		int margin_top=getPingmu().heightPixels/2-share_view_hight/2;
		LinearLayout.LayoutParams layoutParams=new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,share_view_hight);
